...
CUIPoint::CUIPoint(const POINT& src)
{
   x = src.x;
   y = src.y;
}

CUIPoint::CUIPoint(int _x, int _y)
{
   x = _x;
   y = _y;
}

CUIPoint::CUIPoint(LPARAM lParam)
{
...
{
   TCHAR szBuffer[256];
   int cchLen = ::LoadString(CUIPaintManager::GetLanguageInstance(), nRes, szBuffer, lengthof(szBuffer) - 1);
   ASSERT(cchLen>0);
   szBuffer[cchLen] = '\0';
   return szBuffer;
}
...
HWND CWindowWnd::Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, int x, int y, int cx, int cy, HMENU hMenu)
{
	...
	m_hWnd = ::CreateWindowEx(dwExStyle, GetWindowClassName(), pstrName, dwStyle, x, y, cx, cy, hwndParent, hMenu, CUIPaintManager::GetResourceInstance(), this);
	ASSERT(m_hWnd!=NULL);
	return m_hWnd;
}
...
{
	HICON hIcon = (HICON)::LoadImage(CUIPaintManager::GetResourceInstance(), MAKEINTRESOURCE(nRes), IMAGE_ICON, ::GetSystemMetrics(SM_CXICON), ::GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR);
	ASSERT(hIcon);
	::SendMessage(m_hWnd, WM_SETICON, (WPARAM) TRUE, (LPARAM) hIcon);
	hIcon = (HICON)::LoadImage(CUIPaintManager::GetResourceInstance(), MAKEINTRESOURCE(nRes), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON), ::GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR);
	ASSERT(hIcon);
...
bool CWindowWnd::RegisterWindowClass()
{
	...
	wc.hInstance = CUIPaintManager::GetResourceInstance();
...
bool CWindowWnd::RegisterSuperclass()
{
	...
	if( !::GetClassInfoEx(NULL, GetSuperClassName(), &wc) ) {
		if( !::GetClassInfoEx(CUIPaintManager::GetResourceInstance(), GetSuperClassName(), &wc) ) {
