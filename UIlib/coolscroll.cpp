...
static void PaintRect(HDC hdc, RECT *rect, COLORREF color)
{
	...
	{
		COLORREF oldcol = SetBkColor(hdcmem, color);
		ExtTextOut(hdcmem, 0, 0, ETO_OPAQUE, &rc, _T(""), 0, 0);
		SetBkColor(hdcmem, oldcol);
	}
...
LRESULT CALLBACK CoolSBWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	//case WM_VSCROLL:
	//	{
	//		return ::SendMessage(GetParent(hwnd), WM_VSCROLL,wParam, (LPARAM)hwnd);
	//	}
		break;
