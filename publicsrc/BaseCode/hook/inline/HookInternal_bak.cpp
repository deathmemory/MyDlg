...
DWORD HOOK_INTERNAL::SearchKeyCode(DWORD Pid,DWORD searchFrom,char *KeyCode,DWORD searchSize)
{
	...
	if(!searchSize)searchSize=0x00800000;
	...
	if(Pid==GetCurrentProcessId())
	{
		...
		ret = SearchKeyStr((BYTE*)cmpcode,KeyCode,lstrlenA(KeyCode),searchSize-4);
	}else
	{
...
DWORD HOOK_INTERNAL::SearchKeyStr(BYTE* pbegin,CHAR *fcode,ULONG lenCode,ULONG searchLen)
{
	...
	for(cntSearch=0;cntSearch<searchLen;cntSearch++)
	{
		for(cntCmp=0;cntCmp<lenCode;cntCmp+=2)
		{
			...
			if(pCode[cntSearch+(cntCmp>>1)]==(BYTE)ConvertHexToDec((CHAR*)(ptr+cntCmp),1))
			{
				if(cntCmp+2==lenCode){return (DWORD)(BYTE*)(pbegin+cntSearch);}
			}else{break;}
